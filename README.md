...
- Группировка по категориям (Dev / Applications / System) с цветными заголовками
- Определение фреймворка (Next.js, Vite, Python, Express и др.)
- Отображение uptime каждого сервера
- Остановка сервера одним кликом (с подтверждением для системных процессов)
- Открытие в браузере одним кликом
- Сворачивание в системный трей (работает в фоне)
- Автозапуск при старте Windows (toggle в StatusBar)
- Дедупликация IPv4/IPv6 портов
- CSP, валидация портов, безопасный kill

## Статус

v0.2.0 — tray mode, autostart, kill confirmation.
